...
inline.footnote = function(buffer, item)
  ...
  local text = { string.byte(string.sub(item.text, 3, -2), 0, -1) }
  local text_new_table = vim.tbl_map(function(x)
    if x == 0 then return nil end
    if x == 49 then return string.char(194, 185) end
    if x >= 50 and x <= 51 then return string.char(194, x + 128) end
    return string.char(226, 129, x + 128)
  end, text)

  local text_string = table.concat(text_new_table)
  ...
  vim.api.nvim_buf_set_extmark(buffer, inline.ns, range.row_start, range.col_start, {
    ...
    virt_text_pos = "inline",
    virt_text = {
      { text_string, utils.set_hl(config.hl) },
    },
    hl_mode = "combine"
  });
...
inline.link_internal = function(buffer, item)
  ...
  vim.api.nvim_buf_set_extmark(buffer, inline.ns, range.row_start,
