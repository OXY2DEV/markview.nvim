...
- Simple checkbox state cache.
  This module can remember previous checkbox states(even after they are removed) so that you can simply toggle them without needing to manually change the state.

- Interactive mode.
  Allows changing checkbox state(on the current line) via `hjkl` interactively.
